...
function colorful_tag_cloud_rep ($content) 
{
	if (strstr($content,"[colorful_tag_cloud]") ) 
	{
		$options = (array) get_option('ColorfullTagCloud');
		...
		//print_r($content);die();
		$toprint.= '<p class="catcloud" style="text-align:'.$options['align'].';">';
		...
		if (!$options['hide-cats'] && !$options['hide-tags'])	
			$toprint.= 'Please cofigure properly...';
			
		$toprint.= '</p>';
		
		if (!$options['hide-poweredby'])
			$toprint.= '<p style="font-size:xx-small;font-style:italic;text-align:right;"><a href="http://www.techxperts.co.in/wordpress/plugins/category-cloud-widget/">-- Powered by Techxperts</a></p>';
		
		$toprint.= $after_widget;
		$content=str_replace("[colorful_tag_cloud]",$toprint,$content);
		return $content;
	}
	else
	{
...
add_filter("the_content", "colorful_tag_cloud_rep", 10);
?>